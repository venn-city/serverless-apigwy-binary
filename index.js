...
class ApiGwyBinaryPlugin {
  ...
  setContentHandling(funcs) {
    ...
    const apiId = this.serverless.service.provider.apiGateway.restApiId;
    ...
    integrationResponse.restApiId = apiId;

    return (
      apigateway
        .getRestApis()
        .promise()
        .then(() => apigateway.getResources({ restApiId: apiId }).promise())
        .then(resources => {
            ...
            funcs[fKey].events.forEach(e => {
              if (e.http && e.http.contentHandling) {
                ...
                integrationResponse.resourceId = resources.items.find(
                  r => r.path === `/${e.http.path}`
                ).id;

                integrationPromises.push(
                  apigateway
                    .putIntegrationResponse(integrationResponse)
                    .promise()
                );
              }
            });
          ...
          this.serverless.cli.log(
            "Setting up content handling in AWS API Gateway (takes ~1 min)..."
          );
        ...
        .then(
          () => new BbPromise(resolve => setTimeout(() => resolve(), 60000))
        )
        .then(() => {
          this.serverless.cli.log(
            "Deploying content handling updates to AWS API Gateway..."
          );
          return apigateway
            .createDeployment({
